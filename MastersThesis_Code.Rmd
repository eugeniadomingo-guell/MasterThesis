...
colors_sam <- c("Adjacent" = "#8DBBDC", "Control" = "#358747", "Tumor" = "#C81518", "CNE" = "#FA9D4F" , "No_pathologic" = "#F4D166", "Patient" = "#C590B3")
...
# Loading metadata
mapping_file <- read.delim("/path/working/directory/mapping_file.tsv")  #127 patients
## Homogenizing metadata with metadata from microbiome analysis
cnagID <- read.delim("/path/working/directory/cnagID.txt")
samples <- read.delim("/path/working/directory/samples.txt")
```
...
# Determining age:
## Cases:
Casos_pancrees <- read.csv("/path/working/directory/Casos_pancrees_Eugenia.csv")
...
## Age for CONTROLS:
FFIDs_ages <- read.csv("/path/working/directory/FFIDs_ages.csv", sep=";")
Controls_ages <- read.csv("/path/working/directory/Controls_ages.csv", sep=";")
...
metadata_final
#write.csv(metadata_final, "/path/working/directory/metadata_final.csv")
...
There are 86 individuals who participated in this study, as pancreatic samples donnors. 
...
#write.csv(meta_part, "/path/working/directory/metadata_participants.csv")
```


### Group 

There are 32 controls and 54 pancreatic cancer patients in this study. 
...
### Loading Raw counts: 
rawCounts <- read.delim("/path/working/directory/rawCounts", row.names=1, check.names = FALSE)
### Loading TPM counts:
TPM_counts <- read.delim("/path/working/directory/TPM_counts", row.names=1, check.names = FALSE)
```
...
#write.csv(rawCounts_2, "/path/working/directory/RawCounts2.csv")
...
annot <- read.table("/path/working/directory/gencode.v26.GRCh38.ERCC.genes.info.txt", header = TRUE, stringsAsFactors = FALSE)   #56,292 genes in the annotation file 
### Annotation file: downloaded on Jul 27, 2020

annot2 <- annot[annot$gene_type == "protein_coding", ] #19,291 protein coding genes

noprot_genes <- rawCounts_2[! rownames(rawCounts_2) %in% annot2$gene_id, ] ## 39 020 non-protein coding genes 
protcod_genes <- rawCounts_2[rownames(rawCounts_2) %in% annot2$gene_id, ] ### 19291
```
...
ls <- apply(protcod_genes, 2, sum)/10^6 #apply the sum of values in each row. 
range_ls <- range(ls)  #  7.175031 55.786910

#The values of the total reads per sample vary from 7.175031 reads to 55.786910 between different samples. 
...
# Detect which sample is the one that has a ls = 55.7869
sampleTDF$total <- as.numeric(sampleTDF$total)
ls_max <- sampleTDF[which.max(sampleTDF$total), ]  ### sample FF90

pls
```
Bar plot representing the library size (million counts) across all 124 samples of the study. The colors of the bars range from dark blue (lowest size) to light blue (biggest library size).
...
genes_after_fil_1 <- nrow(counts_filtered)  
genes_after_fil_2 <- nrow(counts_filt_f)   

rejected_genes <- nrow(protcod_genes)-nrow(counts_filt_f) 
...
countsTMM <- cpm(Norm.Factor, log = T)  #log = TRUE !!!  important parameter do describe in the methods. Log2 values. 
countsTMM_head <- (countsTMM)[1:5, 1:5]



countsTMM.df <- as.data.frame(countsTMM)
rownames(countsTMM.df) <- gsub("\\..*$", "", rownames(countsTMM.df))
#write.csv(countsTMM.df, "/path/working/directory/normalized_counts_2.csv")
...
Hierarchical Clustering of the samples (normalized by TMM), by Ward.D2 method. Samples labeled based on their sex ("M" for male and "F" for female) and according to their group ("C" for Control, "A" for adjacent or "T" for tumor).
...
In this case, the normalized TMM counts are used to undergo the PCA.

```{r PCA, echo=TRUE}
metadata_final <- read.csv("/path/working/directory/metadata_final.csv", row.names=1)
metadata_final$Group <- as.factor(metadata_final$Group)
metadata_final$Sex <- as.factor(metadata_final$Sex)
metadata_final$Age <- as.numeric(metadata_final$Age)

metadata_final[order(metadata_final$FF_ID), ]
#meta_rna [order(meta_rna$FF_ID), ]
protcod_genes[,order(colnames(protcod_genes))]
...
Principal Components Analysis of the sample of study, colored by group of samples and shaped by sex of the patients. The PC1 is represented in the X axis, and the PC2 is projected in the Y axis.

The samples belonging to the Control group are clustering closely, with low variability in PC2 and wider distribution in PC1. Control and Adjacent samples cluster together within the same ellipse, with wide distribution across PC1 and PC2.
...
res_ct[! is.na(res_ct$padj) & res_ct$padj < 0.05 & abs(res_ct$log2FoldChange) > 3, ]    ### 1058 DEG 
...
#write.csv(deg_ct, "/path/working/directory/DEGs_CT.csv")

### Adjacent vs Control
res_ca <- results(dds_2, contrast = c("Group", "Adjacent", "Control"))  
res_ca[!is.na(res_ca$padj) & res_ca$padj < 0.05 & abs(res_ca$log2FoldChange) > 3, ]    ### 310 DEG
...
#write.csv(deg_ca, "/path/working/directory/DEGs_CA.csv")

### Tumor vs Adjacent
res_ta <- results(dds_2, contrast = c("Group", "Tumor", "Adjacent"))  
res_ta[!is.na(res_ta$padj) & res_ta$padj < 0.05 & abs(res_ta$log2FoldChange) > 3, ]    ### 512 DEG
...
#write.csv(deg_ta, "/path/working/directory/DEGs_TA.csv")
...
The expression levels and distribution status of all DEGs in the dataset are presented in a heatmap.
...
cat("The principal Hallmark Gene Sets that has been significantly associated with the Differential Expressed Genes comparing Adjacent vs Control is the", em.ca$ID[1] , ", the", em.ca$ID[2], ", and the", em.ca$ID[3], ".")
...
res.ct.df <- as.data.frame(res_ct) 
...
Bar plot representing the count of differential expressed genes that are associated with the significantly associated Hallmark gene sets.

In the context of analyzing gene ontology terms, the p-value is commonly used to assess the enrichment or over-representation of specific GO terms within a set of genes.



```{r}
em.ct.df <- data.frame(GS = em.ct$ID, GR = em.ct$GeneRatio, Gene = em.ct$geneID)
write.csv(em.ct.df, "/path/working/directory/Hgenes_CT.csv")

em.ca.df <- data.frame(GS = em.ca$ID, GR = em.ca$GeneRatio, Gene = em.ca$geneID)
write.csv(em.ca.df, "/path/working/directory/Hgenes_CA.csv")

em.ta.df <- data.frame(GS = em.ta$ID, GR = em.ta$GeneRatio, Gene = em.ta$geneID)
write.csv(em.ta.df, "/path/working/directory/Hgenes_TA.csv")
```
...
write.csv(hallmark_count_ct, "/path/working/directory/hallmark_count_CvsT.csv")
...
write.csv(hallmark_count_ta, "/path/working/directory/hallmark_count_AvsT.csv")
...
write.csv(hallmark_count_ca, "/path/working/directory/hallmark_count_CvsA.csv")
```




Another way to visualize the results, can be with a dot plot, like in Figure 9.
...
Dotplot representing the Gene Ratio of DEGs that are associated with the significantly associated Hallmark gene sets: *Allograft Rejection*, *Interferon Gamma response*, and *Inflammatory Response*. There is a color code raging from red to blue, representing the significance by which these hallmark gene sets are associated with the DEGs. There is a difference in the sizes of the dots, depending on the number of gene counts that were associated by each of them.

The **Gene-Concept Network Plot** is a useful tool to identify the DEGs that are involved with the significant hallmark gene sets. Additionally, it is possible to visualize if one of the genes is related to more than one biologic hallmark gene set.
...
Gene-concept network plot of the identified hallmark gene sets and the Differential Expressed Genes that are involved. The Hallmark gene sets  are represented with brown circles, of different size depending on the number of genes that are associated with them. The name of the DEGs are represented with a color range (raging from red to blue) depending of their relative expression (the value of the log fold change). The genes are connected to their corresponding gene sets by grey lines.

The Gene-Concept Network Plot represents the relationship between the significant hallmark gene sets that have been identified and the differential expressed genes in pancreatic cancer comparing tumor, control and adjacent tissue. 

In this study, the identified DEGs for each of the significant hallmarks are:

And there are some genes that are common in various hallmarks.


A similar way to visualize the relative expression of the DEGs between males and females and the hallmark gene sets in which they are implicated, is by representing a heatplot.
...
Heatplot showing the identified differential expressed genes in the columns, and the significant hallmark gene sets in the rows. 
...
genenames.df <- as.data.frame(count_gn) # 12268 genes

Symbol.2 <- Symbol[!duplicated(Symbol)] # 13476 genes
Symbol.2 <- Symbol.2[!is.na(Symbol.2)] # 12166 genes
...
pheatmap(gsva, cluster_cols = TRUE, cluster_rows = TRUE, clustering_method = "ward.D2",
          scale="row", cellheight = 4.5, fontsize_col = 2.5, fontsize_row = 4, 
...
Heatmap with the evaluation of the representation of the gene set enrichment hallmarks for each sample. The color code ranges from blue to red, and it represents under-representation or over-representation of the genes from that hallmark collection as DEGs.
...
deconv_tumor <- deconv_tumor[!duplicated(deconv_tumor$Symbol), ] 
...
# Normalization
deconv_tumor_norm <- DGEList(deconv_tumor)   
keep_dt <- rowSums(cpm(deconv_tumor_norm)>0)>=1
deconv_tumor_norm <- deconv_tumor_norm[keep_dt, , keep.lib.sizes=FALSE] 
...
median(as.numeric(tum_res_quantiseq[tum_res_quantiseq$cell_type == "uncharacterized cell",-1])) ## 0.62159
mean(as.numeric(tum_res_quantiseq[tum_res_quantiseq$cell_type == "uncharacterized cell",-1])) ## 0.6032
...
length(genes.adj) # 12268
length(unique(genes.adj))  # 12268
deconv_adj <- deconv_adj[!duplicated(deconv_adj$Symbol), ]  #12268 genes
...
# Normalization
deconv_adj_norm <- DGEList(deconv_adj)   ### 12268 genes and 39 samples ### 
keep_da <- rowSums(cpm(deconv_adj_norm)>0)>=1
deconv_adj_norm <- deconv_adj_norm[keep_da, , keep.lib.sizes=FALSE] # 12266 genes
...
length(genes.ctrl) #12268
length(unique(genes.ctrl))  # 12167
deconv_ctrl <- deconv_ctrl[!duplicated(deconv_ctrl$Symbol), ]  
...
# Normalization
deconv_ctrl_norm <- DGEList(deconv_ctrl)   
keep_dc <- rowSums(cpm(deconv_ctrl_norm)>0)>=1
deconv_ctrl_norm <- deconv_ctrl_norm[keep_dc, , keep.lib.sizes=FALSE] # 
...
raw_tumor_counts <- counts_filt_f[, colnames(counts_filt_f) %in% tumors$FF_ID] #raw tumor counts  --> 12268 genes
...
tumor_counts <- tumor_counts[!duplicated(tumor_counts$Symbol), ]  #12166 genes
tumor_counts <- tumor_counts[! is.na(tumor_counts$Symbol), ]
rownames(tumor_counts) <- tumor_counts$Symbol

tumor_counts <- tumor_counts[, ! colnames(tumor_counts) %in% c("Symbol")]  # 12166 genes --> Gene symbol 
```
...
Data was normalized using TMM (weighted trimmed mean of M-values) as implemented in the R package ‘edgeR’. For downstream analyses, normalized RSEM data were converted to counts per million (c.p.m.) and log2 transformed35. Genes without at least 1 c.p.m. in 20% of the sample were excluded from further analysis.


### Bailey

Transcriptomic subtypes (REVIEW, doi:https://doi.org/10.1038/s41575-019-0109-y). 
...
Squamous subtype: included gene networks involved in inflammation, hypoxia response, metabolic reprogramming, TGF-β signalling, MYC pathway activation, autophagy and upregulated expression of TP63∆N and its target genes. 
...
gp1 <- readxl::read_xlsx("/path/working/directory/41586_2016_BFnature16965_MOESM273_ESM.xlsx", sheet = 1)
gp1 <- gp1$Nodes
genes_gp1 <- unique(unlist(lapply(gp1, function(x) unlist(strsplit(x, split=",")))))

gp10 <- readxl::read_xlsx("/path/working/directory/41586_2016_BFnature16965_MOESM273_ESM.xlsx", sheet = 2)
gp10 <- gp10$Nodes
genes_gp10 <- unique(unlist(lapply(gp10, function(x) unlist(strsplit(x, split=",")))))

gp2 <- readxl::read_xlsx("/path/working/directory/41586_2016_BFnature16965_MOESM273_ESM.xlsx", sheet = 3)
gp2<- gp2$Nodes
genes_gp2 <- unique(unlist(lapply(gp2, function(x) unlist(strsplit(x, split=",")))))

gp3 <- readxl::read_xlsx("/path/working/directory/41586_2016_BFnature16965_MOESM273_ESM.xlsx", sheet = 4)
gp3<- gp3$Nodes
genes_gp3 <- unique(unlist(lapply(gp3, function(x) unlist(strsplit(x, split=",")))))

gp4 <- readxl::read_xlsx("/path/working/directory/41586_2016_BFnature16965_MOESM273_ESM.xlsx", sheet = 5)
gp4<- gp4$Nodes
genes_gp4 <- unique(unlist(lapply(gp4, function(x) unlist(strsplit(x, split=",")))))

gp5 <- readxl::read_xlsx("/path/working/directory/41586_2016_BFnature16965_MOESM273_ESM.xlsx", sheet = 6)
gp5<- gp5$Nodes
genes_gp5 <- unique(unlist(lapply(gp5, function(x) unlist(strsplit(x, split=",")))))

gp6 <- readxl::read_xlsx("/path/working/directory/41586_2016_BFnature16965_MOESM273_ESM.xlsx", sheet = 7)
gp6<- gp6$Nodes
genes_gp6 <- unique(unlist(lapply(gp6, function(x) unlist(strsplit(x, split=",")))))

gp7 <- readxl::read_xlsx("/path/working/directory/41586_2016_BFnature16965_MOESM273_ESM.xlsx", sheet = 8)
gp7<- gp7$Nodes
genes_gp7 <- unique(unlist(lapply(gp7, function(x) unlist(strsplit(x, split=",")))))

gp8 <- readxl::read_xlsx("/path/working/directory/41586_2016_BFnature16965_MOESM273_ESM.xlsx", sheet = 9)
gp8<- gp8$Nodes
genes_gp8 <- unique(unlist(lapply(gp8, function(x) unlist(strsplit(x, split=",")))))

gp9 <- readxl::read_xlsx("/path/working/directory/41586_2016_BFnature16965_MOESM273_ESM.xlsx", sheet = 10)
gp9<- gp9$Nodes
...
#write.csv(Baileysig, "/path/working/directory/Bailey_signature.csv")
...
B_sign <- Baileysig[Baileysig$gene%in%row.names(bailey_expr), ]  #1370 genes

# PDA signature heatmap
pal <- colorRampPalette(c("blue", "white", "red"))(264)
# scaling+centering bailey's gene expression
Bai_genes <- bailey_expr[rownames(bailey_expr) %in% B_sign$gene, ]  #1370   53
...
subtypes_Zscores <- sapply(Bai_subtypes, function(genes) {
  ...
  common_genes <- intersect(genes, rownames(Bai_scaled))
    # Compute the column means (average expression) for those genes
  colMeans(Bai_scaled[common_genes, -ncol(Bai_scaled) , drop = FALSE])
})
...
#write.csv(subtypes_Zscores, "/path/working/directory/Prediction_Bailey.csv")
...
16S rRNA gene amplicon sequencing is most probably done under the hypervariable region V4 of the 16S rRNA sequence. These hypervariable regions are very specific for strains, and are commonly used for taxonomic identification of bacteria.

In most bacterial species, 16S rRNA V4 regions are usually 254 bp long. (!!!)

## DADA2

DADA2 implements an algorithm that models the errors introduced during amplicon sequencing, and uses the error model to infer the true sample composition. DADA2 infers sample sequences exactly, without coarse-graining into OTUs, and resolves differences of as little as one nucleotide. (!!! doi: 10.1038/nmeth.3869). It is used as a quality control process to evaluate the quality of the reads of the 16S rRNA sequencing.
...
#Read fastq files: 
data_path <- ("/path/working/directory/fastq") #Working directory of the raw files
...
#There are the same sample names in the forward and the reverse filenames. 
# Good
```
...
#Trimming at 250 bp of length for forward sequences 
# Good
```
...
filt_path_R1 <- file.path("/path/working/directory/filtered_R1") #Working directory of the raw files
filt_path_R2 <- file.path("/path/working/directory/filtered_R2") #Working directory of the raw files
...
#saveRDS(dada_F, "/path/working/directory/dada_F.RData")

dada_R <- dada(filt_R, err=err_R, multithread=TRUE)
#head(dada_R$FF1_R_filt.fastq.gz)
#saveRDS(dada_R, "/path/working/directory/dada_R.RData")
...
#saveRDS(merged, "/path/working/directory/merged.RData")
...
This table contains 29506 ASVs, and a 58.89% of the lengths of our merged sequences fall within the expected range for this V4 amplicon: between 250 and 310 bp long.
...
#write.csv(seqtab_nochim, "/path/working/directory/filt_reads.csv")
...
# write.csv(track, "/path/working/directory/track_filt_reads.csv")
```
...
track <- read.csv("/path/working/directory/track_filt_reads.csv", row.names=1)
...
g_filt_reads
```
Barplot with the mean count of reads across all samples that have been filtered after each step of the DADA2 pipeline.

In general, around 10% of the input reads passed all the filtering steps of the quality control pipeline of DADA2. Let's calculate the percentage of reads that passed the filtering steps.
...
track_no_qc <- track[!rownames(track) %in% c("CN076", "CNE", "CNE76", "CNLane", "FF21", "FF25", "Mock1",  "Mock2", "Mock3"), ] #130 samples, good! 
...
#write.csv(track_no_qc, "/path/working/directory/track_reads_posqc.csv")
```

By mean, an 11,36% of the reads passed the filtering steps done by the quality control pipeline of DADA2.

The samples: XXXXX don't meet the quality control criteria: in the end of the DADA2 pipeline, these samples contain less than 1% of the input reads.
...
#write.csv(table_1, "/path/working/directory/Mean_reads_tracking.csv")
```
...
# Loading metadata
meta_ps <- read.csv("/path/working/directory/final_metadata.csv", sep= ",", header = T, row.names = 1)
## 127 samples --> Good
## Contains CNE and the samples not passing the DADA2 qc are removed. 

# Output file generated by Protocol 4:
asv_table_16S <- readRDS("/path/working/directory/asv_table_16S.rds")
dim(asv_table_16S) 
...
identical(rownames(sample_ps), rownames(asv_table)) #TRUE! Good, we can proceed to create the phyloseq object 
...
# saveRDS(ps, file="/path/working/directory/ps.rds")
# saveRDS(ps_genus, file="/path/working/directory/ps_genus.rds")
# saveRDS(ps_species, file = "/path/working/directory/ps_spp.rds")
...
Histogram with the frequency of the library counts from all the samples included in the phyloseq object. 

The histogram shows a good distribution of the total number of reads after filtering (for the ps object). The sample with the lowest count of reads is sample  CN276, which presents 0 reads. Besides negative controls, the next sample with the lowest number of counts is FF24, which presents 2 identified reads with a taxa. The sample FF70 is the one that presents the most reads to analyze: 25401 reads with an assigned taxa.
...
Histogram with the frequency of the library counts from all the samples included in the ps_species object.

A lot of samples lose completely their reads when filtering to species and genus level.
...
Box plot with the distribution of the total number of reads depending on the group of the sample. The Kruskal-Wallis statistical test is used to test significant differences between the 4 groups. 
...
#saveRDS(ps_clean, file="/path/working/directory/ps_clean.rds")

write.csv(as.data.frame(ps_clean@tax_table), "/path/working/directory/identified_taxa.csv")
...
Scatter plot with the correlation of the sample weight (mg) and the library size of each of the samples that passed the quality control. The correlation test in Spearman's method reveals a no-significant correlation between the variables.
...
The mirror plot displays no observable correlation between library size and sample weight. The Spearman's correlation test provides no significant results on the correlation between library size of the samples and sample weight.
...
In order to allow a proper Shannon Index comparison, the differences were adjusted regarding the number of reads across the samples rarefying to the minimum number of reads. The Rarefaction Efficiency Index was X. As Chao 1 takes into account the unobserved species, it was calculated directly from raw abundance data.
...
No significant differences in the alpha diversity measurements between groups. 
...
In this study, the Multiple Factor Analysis has been done by the `MFA` function from the `FactoMineR` package (v 2.11), and executed in R (v 4.5.1). The function has been applied over 3 blocks of data:

-   The type of sample, which is a single categorical variable with 3 levels: Control, Adjacent and Tumor.
-   Gene expression: a block composed by 13542 features, which correspond to genes in a ENTREZID format and their expression values across all samples. This is a continuous variable, which has been Normalized under "TMM" method, Log2 transformed, and filtered by coefficient of variation.
-   Microbiome: a block composed by 19 features, which correspond to bacterial taxonomies and their counts values across all samples. 

In total, the prepared data that was introduced into the MFA function contained information for XXX features from 122 samples.
...
rna_n <- read.csv("/path/working/directory/normalized_counts.csv", row.names=1)  # 12268
## Standarizing gene symbols
rna_n$Symbol <- mapIds(org.Hs.eg.db, keys = rownames(rna_n), keytype = "ENSEMBL", column = "SYMBOL")
genes <- rna_n$Symbol
genes.u <- unique(genes) #12167
rna_n.f <- rna_n[!duplicated(rna_n$Symbol), ]  #12167
rna_n.f <- rna_n.f[!is.na(rna_n.f$Symbol), ]   # 12166
...
rna_dft <- as.data.frame(t(rna_df))   ### 12166 genes (columns)
mfa_rna <- rna_dft   ### 124 samples (rows) and 12166 columns (genes)


### MICROBIOME DATA
phyloseq_microbiome <- readRDS("/path/working/directory/ps_clean.rds")
...
n_total <- n_group + n_rna + n_microb_ord   ### 12183 columns
...
dim(mfa_df_f) ### 122 rows and 12183 columns

#write.csv(mfa_df_f, "/path/working/directory/MFA_df_ALL_ORDER.csv")
...
#                     Dim.1      Dim.2      Dim.3
# Group          0.9134843 0.66358771 0.32273318
# mRNA           0.9022424 0.09630856 0.05626518
# Ord_Microbiome 0.0771520 0.38556994 0.61083260

mfa_build_ord$group$contrib
#                   Dim.1     Dim.2     Dim.3
# Group          48.258999 57.931671 32.604878
# mRNA           47.665092  8.407804  5.684322
# Ord_Microbiome  4.075909 33.660525 61.710800

mfa_build_ord$group$RV
#                     Group       mRNA Ord_Microbiome       MFA
# Group          1.00000000 0.48509514     0.03683511 0.7839773
# mRNA           0.48509514 1.00000000     0.07552324 0.7452283
# Ord_Microbiome 0.03683511 0.07552324     1.00000000 0.5146759
# MFA            0.78397729 0.74522832     0.51467589 1.0000000
...
cat("The features that have more impact on the Methanobacteriales:", paste0(rownames(vars.meth)[-nrow(vars.meth)], sep=","),
 "which are located in closest coordinates in the Global PCA after the Multiple Factor Analysis representation.")
### DBNDD1, SOX8, GIPR, DDX11, ACP3, IL20RA, MOK, AACS, MECOM, TRIP6, TESC, DZANK1, CFAP61, RAB36, TELO2, ARFGAP1, TRIB3, SYTL4, IQCH, JPH1, ARFGEF3, KCNC4, KCNQ4, ZNF205, SPATA1, KCNK15, PLGLB2, BFSP1, TRAP1, AJUBA, GDF15, ZNF341, TUBG1, PSMC3IP, ADAM20, SLC49A4, TSSK4, TPM1, SLC5A2, LRRC46, NBPF3, OAZ3, DUSP10, PSD2, SPIN2A, ARHGAP39, KLHL35, CIMIP5, MIPOL1, KCTD14, CMTM7, NEIL2, PIK3AP1, GRIP1, CFAP161, CREB3L1, CFAP251, UBXN11, RNF207, CLDN14, ALDH4A1, CLPB, PRKAA2, FBXO41, PLSCR2, UCN, CDCA7L, MID1IP1, NLGN4Y, AK8, NUDT13, USP54, ANKS3, MTCL1, NPIPB3, FASN, KRT86, NPAS2, HSFX1, SNTB1, PRSS27, FGFBP3, PNPLA2, CEP295NL, ZNF320, PYCR1, PRR36, SRPK3, CLDN6, SGCZ, NRBP2, KLHL17, HES4, CCDC9B, ALKBH2, CFAP119, PLEKHG4, ADAM32, GOLGA6L9, KEL, C2CD4A, FOXO6, CFAP95, PCDHGA1, DENND6B, TSSK1B, CRYGS, CSKMT, NPIPA7, FAM200C, C2CD4D, PCDHGC5, ARHGAP8, PWP2, PCDHGC4, CNTF, ZNF709, OR2AE1, MISFA, ATP5MGL, PCDHGA12, PCDHGA11, PCDHGB2, PCDHGB7, RTEL1, HSPB9, PCDHGA9, NOTCH2NLA, UPK3BL1, ZBTB8B, which are located in closest coordinates in the Global PCA

dim(vars.meth) ### 131 genes

rownames(vars.meth)[rownames(vars.meth) %in% deg_ct$Symbol]
# "SGCZ"   

deg_ct[deg_ct$Symbol == "SGCZ", ]  # log2FC = -3.43645


genes_methano <- enricher(rownames(vars.meth)[-length(rownames(vars.meth))], TERM2GENE=H.symbol)
...
Prediction_Bailey <- read.csv("/path/working/directory/Prediction_Bailey.csv", row.names=1)
...
n_predBaileys <- 1
n_rna_t <- ncol(mfa_rna_tumor) #12166
n_microb_t <- ncol(mfa_otu_tumor_ord) #16
n_total_t <- n_predBaileys + n_rna_t + n_microb_t # 12183 columns
...
dim(mfa_df_tumor_ord)   ### 53 rows = 53 tumor samples
                        ### 12183 columns  = 12183 features
#write.csv(mfa_df_tumor_ord, "/path/working/directory/MFA_df_TUMOR_ORDER.csv")
...
#                    Dim.1      Dim.2     Dim.3
# Pred_Bailey    0.7185744 0.95448724 0.8771415
# mRNA           0.8491367 0.39868987 0.1368216
# Ord_Microbiome 0.2676694 0.02596068 0.1576592

mfa_build_tumor_ord$group$contrib
#                   Dim.1     Dim.2    Dim.3
# Pred_Bailey    39.15125 69.208983 74.86555
# mRNA           46.26488 28.908632 11.67796
# Ord_Microbiome 14.58387  1.882384 13.45649

mfa_build_tumor_ord$group$RV
#                Pred_Bailey      mRNA Ord_Microbiome       MFA
# Pred_Bailey     1.00000000 0.4181173     0.02968541 0.7951948
# mRNA            0.41811734 1.0000000     0.17694956 0.7632790
# Ord_Microbiome  0.02968541 0.1769496     1.00000000 0.4809263
# MFA             0.79519481 0.7632790     0.48092633 1.0000000
...
res_betatum 
#write.csv(res_betatum, "/path/working/directory/beta_tum.csv")
...
No differential taxonomic abundance between bailey's predicted subtypes. No significant tests either by KW and limma. 
